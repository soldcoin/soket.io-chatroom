<template>
	<div class="room-container">
		...
		<div class="chatroom">
			<div v-bind:class="'msgBox ' + (item.isCurrent ?　'outBox' : 'inBox')" v-for="item in msgGroup">
				<div>
...
<script type="text/javascript">
	...
	export default {
	    ...
	    mounted () {
	      ...
	      this.socket.on('receiveMsg', (data) => {      //接收消息
	      	data.isCurrent = false;
	      	this.msgGroup.push(data);
	      });

          this.socket.on('connect', () => {              //监听登录
            console.log('connect fired');
            this.socket.emit('login', this.userdata);
          });

          this.socket.on('reconnect', () => {            //监听登录(重连)
            console.log('reconnect fired');
          });

          this.socket.on('user join', (data) => {        //用户连接提示
            // console.log(data.name + '进入房间');
          });

          this.socket.on('user left', (data) => {        //用户退出提示
